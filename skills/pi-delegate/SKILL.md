...
- To check proactively before a big batch: `pio quota` (exit 0 ok / 2 warn / 3 block).

## Worker reliability (learned from live use)

- The MiniMax API sometimes stalls producing nothing; `pio` kills idle workers after
...
- Maintain the board through `pio task add|assign|start|review|done|drop|move --session ... --actor brain|human`; never write task JSON directly.
- Use `pio send`, `pio retry`, and `pio handoff` for durable intervention rather than inventing runner flags. The sole runner is Rust; bounded logs still apply.
