...
## Integration and console validation

- The v2 registry can describe an idle timeout but has no durable distinction for a
  provider ending normally because it exhausted the context/token allowance. The Rust
  runner now recognizes only explicit provider signatures and marks the run
  `attention: handoff_needed`; it does not guess from an incomplete-looking answer.
  The brain must still inspect the checkpoint and supply the remaining-work brief.
- Session summary status originally inherited the highest-attention child, causing a
  live three-worker session to display `failed` because one older worker timed out.
  The detail was truthful but the group headline was operationally wrong. The group
  now stays `running` while any child is live and retains the failure in its attention
  panel.
- CLI help generated by the first Clap pass listed commands without descriptions.
  That made headless discovery depend on the README or TUI footer; concise Clap help
  text was added before handoff.
- The first VHS capture was monochrome because the parent Codex environment exported
  `NO_COLOR=1`. The capture recipe now unsets it explicitly. This was a capture issue,
  not a theme fallback bug; real terminals still respect `NO_COLOR` intentionally.
